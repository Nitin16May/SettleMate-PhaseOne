...
function buildPDF(tripid, dataCallback, endCallback) {
	Trip.findById(tripid)
		...
		.then(x => {
			...
			doc.fontSize(30).text(x.name);
			doc.fontSize(15).text('Mananged By : ' + x.owner.name + ' | ' + x.owner.email);
			doc.moveDown();
			let tripcost = 0;
			for (let i in x.transactions) {
				tripcost = tripcost + x.transactions[i].amt;
			}
			doc.fontSize(15).text('Total Trip Expense : ' + tripcost);
			doc.moveDown();
			doc.fontSize(15).text('Members : ');
			const data = new Map();
			const net = new Map();
			for (let i in x.members) {
				doc.fontSize(15).text(' - ' + x.members[i].name + ' | ' + x.members[i].email);
			...
			doc.fontSize(30).text('Minimum Count of Transfers to Settle Up');
			doc.moveDown();
			let tn = 1;
			for (let i in payments) {
				doc.fontSize(20).text('Transfer ' + tn);
				tn = tn + 1;

				doc.fontSize(15).text('Sender : ' + payments[i].sender.name + ' | ' + payments[i].sender.email);

				doc.fontSize(15).text('Amount : ' + payments[i].amt);

				doc.fontSize(15).text('Recceiver : ' + payments[i].receiver.name + ' | ' + payments[i].receiver.email);

				doc.fontSize(15).text('UPI ID : ' + payments[i].receiver.upiid);

				doc.moveDown();
			}
			for (let i in x.members) {
				doc.addPage();
				doc.fontSize(30).text(x.members[i].name + ' | ' + x.members[i].email);
				doc.moveDown();
				let usercost = 0;
				let lended = 0;
				...
				doc.fontSize(15).text('Your Total Expense : ' + usercost);
				doc.moveDown();
				doc.fontSize(15).text('Your Total Reimbursement : ' + lended);
				doc.moveDown();
				doc.fontSize(15).text('Distribution : ');
				doc.moveDown();
				for (let j in x.transactions) {
					if (x.transactions[j].currentStatus == 0) {
						const exists = x.transactions[j].distributedAmong.find(tr => tr.toString() === x.members[i]._id.toString());
						if (exists) {
							...
							if (x.members[i]._id.toString() === x.transactions[j].owner.toString()) {
								...
								doc.fontSize(15).text(' + ' + netamount + ' due to ' + x.transactions[j].name);
								doc.fontSize(15).text(' - ' + netamount1 + ' due to ' + x.transactions[j].name);
							} else {
								doc.fontSize(15).text(' - ' + (x.transactions[j].amt / div).toFixed(2) + ' due to ' + x.transactions[j].name);
							}
						} else if (x.members[i]._id.toString() === x.transactions[j].owner.toString()) {
							let netamount = x.transactions[j].amt.toFixed(2);
							doc.fontSize(15).text(' + ' + netamount + ' due to ' + x.transactions[j].name);
			...
			doc.addPage();
			doc.fontSize(30).text('Active Transactions');
			doc.moveDown();
			for (let i in x.transactions) {
				if (x.transactions[i].currentStatus === 0) {
					doc.fontSize(15).text(
						'Added & Payed By : ' + data.get(x.transactions[i].owner.toString()).name + ' | ' + data.get(x.transactions[i].owner.toString()).email
					);
					doc.fontSize(15).text('Amount : ' + x.transactions[i].amt);
					doc.fontSize(15).text('Reason : ' + x.transactions[i].name);
					doc.fontSize(15).text('Distributed Amoung : ');
					for (let j in x.transactions[i].distributedAmong) {
						doc.fontSize(15).text(
							' - ' +
								data.get(x.transactions[i].distributedAmong[j].toString()).name +
								' | ' +
								data.get(x.transactions[i].distributedAmong[j].toString()).email
						);
					}
					doc.moveDown();
					doc.moveDown();
			...
			doc.addPage();
			doc.fontSize(30).text('Removed Transactions');
			doc.moveDown();
			for (let i in x.transactions) {
			...
			doc.addPage();
			doc.fontSize(30).text('Admin Removed Transactions');
			doc.moveDown();
			for (let i in x.transactions) {
