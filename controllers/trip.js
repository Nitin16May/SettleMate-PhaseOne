...
exports.tripInvite = (req, res, next) => {
	...
	try {
		const y = jwt.verify(encryptedToken, process.env.secretKey);
		console.log(y);
...
exports.trip = (req, res, next) => {
	...
	Trip.findById(tripid)
		...
		.then(x => {
			...
			const invitation = jwt.sign({ tripid: tripid, Version: x.invitationVersion }, process.env.secretKey);
			const invitationLink = `${process.env.backend}/invite/${invitation}`;
			res.render('trip', {
...
exports.postedittrip = (req, res, next) => {
	...
	bothPromises.push(
		Trip.findById(tripid)
			...
			.then(x => {
				x.removeMember(memberTBD).catch(err => {
					console.log(err);
				});
